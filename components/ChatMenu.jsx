...
const ChatMenu = ( {showMenu , setShowMenu} ) => {
  ...
  const {data , users , chats , dispatch ,setSelectedChat} = useChatContext();
    ...
    const handleDelete = async () => {
      try {
        ...
        const filteredChats = Object.entries(chats || {})?.filter(([id,chat]) => id !== data.chatId).sort((a,b) => b[1].date - a[1].date)
  ...
  return (
    ...
    <div className='w-[200px] absolute top-[70px] right-5 bg-c1/[0.8] z-10 rounded-2xl overflow-hidden ' >
         <ul className='flex flex-col rounded-3xl'>

            {!IamBlocked && (<li className='flex items-center py-3 px-5 hover:bg-black cursor-pointer rounded-2xl ' onClick={(e) => {
                 e.stopPropagation() 
            ...
            <li className='flex items-center py-3 px-5 hover:bg-black cursor-pointer rounded-2xl' onClick={(e) => {
              e.stopPropagation()
              handleDelete()
              setShowMenu(false)
            }}>
