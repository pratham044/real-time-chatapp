...
const Avatar = ({size , user , onClick}) => {
    ...
    const c = size === "small" ? "w-8 h-8" : size === "medium" ? "w-9 h-9 " : size === "large" ? "w-10 h-10" : 
    size === "x-large" ? "w-14 h-14" : "w-24 h-24"

    const f = size === "x-large" ? "text-2xl" : size === "xx-large" ? "text-4xl" : "text-base" ;

  return (
    <div className={`${c} rounded-full flex items-center justify-center text-base shrink-0 relative hover:scale-105`}
    ...
    onClick={onClick} >
        ...
        { user?.isOnline && (
            <>
            {size=== "large" && (
                <span className="w-[8px] h-[8px] bg-green-500 rounded-full absolute bottom-[2px] right-[2px] " ></span>
            )}
            {size === "x-large" && (
