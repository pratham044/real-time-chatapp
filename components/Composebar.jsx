...
const Composebar = () => {
  const { inputText , setInputText , data ,attachment ,
    setAttachmentPreview , setAttachment , editMsg , setEditMsg } = useChatContext();
  ...
  return (
    <div className='flex items-center gap-2 grow'>
        <input 
          type='text'
          className='grow w-full outline-0 px-2 py-2 text-white bg-transparent placeholder:text-c3/[0.7]  outline-none text-base'
          placeholder='Type a message ...'
          value={inputText}
          onChange={handleTyping}
          onKeyUp={onKeyUp}
        />
        <button className={`h-10 w-10 rounded-full mr-1 shrink-0 flex justify-center items-center
        ${inputText.trim().length > 0 ? "bg-blue-500" :  "bg-blue-300/[0.3]"}`}
